...
async function processActivities() {
  ...
  const filePath = path.resolve(__dirname, repsActivitiesPath);
  debug(`Getting activities from ${filePath}`);

  let activities = [];
  try {
    activities = JSON.parse(await fs.readFile(filePath, 'utf-8'));
  }
  catch (error) {
    debug('Could not read file content as JSON, aborting..');
    return activities;
  }
