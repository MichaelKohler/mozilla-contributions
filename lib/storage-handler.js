...
class StorageHandler {
  connect() {
    ...
    if (!DB_HOSTNAME || !DB_USERNAME || !DB_PASSWORD || !DB_DATABASE) {
      throw new Error('The following env variables are required: DB_HOSTNAME, DB_USERNAME, DB_PASSWORD, DB_DATABASE');
    }

    this.mysqlConnection = mysql.createPool({
      connectionLimit: parseInt(DB_CONNECTIONS, 10) || 10,
      host: DB_HOSTNAME,
      user: DB_USERNAME,
    ...
    return new Promise((resolve, reject) => {
      this.mysqlConnection.getConnection(async (error) => {
        if (error) {
          return reject(error);
        }
